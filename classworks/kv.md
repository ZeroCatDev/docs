...
namespace用于区分不同用户的空间，原则上其值应为标准的 UUID v4 格式，其不需要预先注册，你需要保证生成的uuid安全性，请不要使用简单随机数自行生成。
key是用于指定资源的标识符，也称为键，其值应为字符串，我们建议您以程序名称作为开头，例如：`classworks-config`和`classworks-20250101`等，这些值应当是可预测的，以便于管理
value是存储在键下的数据，其值必须为JSON
